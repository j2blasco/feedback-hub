...
import { unwrapSuccessResult, resultSuccessVoid } from 'utils/result/results';
import {
  DocumentPath,
  CollectionPath,
  INoSqlDatabase,
} from '../../no-sql-db.interface';
import { testNoSqlDbWrite } from './no-sql-db-write.spec';
...
export const pathTestDocument: DocumentPath = ['test', 'test-document'];
...
export function testNoSqlDb(createDb: () => Promise<INoSqlDatabase>) {
  testNoSqlDbWrite(createDb);
  ...
  describeTest('unit', 'NoSqlDatabase', () => {
    ...
    describe('readDocument', () => {
      it('returns the document when it exists at the given path', async () => {
        const path: DocumentPath = ['test', 'existing-document'];
      ...
      it('returns an error when the document does not exist at the given path', async () => {
        const path: DocumentPath = ['test', 'nonexistent-document'];
